...
void lire_dot(Automate *A, const char *ficher) { 
    ...
    while (fgets(ligne, sizeof(ligne), f)) { 
        ...
        if (sscanf(ligne, " init -> z%d ", &src) == 1) {  //lit des données depuis une chaîne de caractères.
            ajouteretat(A, src); 
    ...
    for (int i = 0; i < A->nbtransitions; i++) { 
        if (A->transitions[i].etatdepart == etat_courant) { 
            ...
            if (symbole == '\0') { 
                ...
                if (mot_accepte_rec(A, A->transitions[i].etatarrivee, mot, 
pos)) 
                    return true; 
